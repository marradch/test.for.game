...
class Comment_model extends CI_Emerald_Model
{
    ...
    {
        $this->path = $path;
        return $this->save('path', $path);
    }

    /**
     * @return int
     */
    public function get_dept(): int
    {
        return $this->dept;
    }
     ...
     * @return bool
     */
    public function set_dept(int $dept)
    {
        $this->dept = $dept;
        return $this->save('dept', $dept);
    }
    ...
    public static function get_all_by_assign_id(int $assting_id)
    {
        ...
        $data = App::get_ci()->s->from(self::CLASS_TABLE)->where(['assign_id' => $assting_id])->orderBy('path','ASC')->many();
        $ret = [];
        foreach ($data as $i)
        {
            $ret[] = (new self())->set($i);
    ...
    public static function get_max_number_by_post($post_id)
    {
        return App::get_ci()->s
            ->from('comment')
            ->where('assign_id', $post_id)
            ->where('dept', 1)
            ->count();
    }

    public static function get_max_number_by_post_and_path($post_id, $path)
    {
        return App::get_ci()->s
            ->from('comment')
            ->where('assign_id', $post_id)
            ->where('path %', $path.'%')
            ->where('dept', count(explode('.', $path)) + 1)
            ->count();
    }

    public static function get_insert_path($post_id, $path = null)
    {
        return $path
            ? $path . '.' . (self::get_max_number_by_post_and_path($post_id, $path) + 1)
            : self::get_max_number_by_post($post_id) + 1;
    }
}
